...
class ProposalGenerator:
    ...
    async def generate(self, draft: DraftIdea) -> Proposal:
        ...
        if self.scopus.is_available:
            articles = await self.scopus.search_for_proposal(
                topic=draft.research_topic[:200], keywords=draft.keywords_en,
            )
        lit_ctx = self._lit_context(articles)

        # 2. Build proposal shell
        proposal = Proposal(
            title_en=draft.title, title_pt=draft.title_pt or draft.title,
            acronym=draft.acronym, typology=draft.typology,
            keywords_en=draft.keywords_en[:4], keywords_pt=draft.keywords_pt[:4],
            scientific_domain=draft.scientific_domain, scientific_area=draft.scientific_area,
        ...
        if draft.pi_career_summary:
            for sec, lim in [("career_profile", CHAR_LIMITS.career_profile),
    ...
    def _fallback_tasks(self, dur: int) -> list[ProposalTask]:
        return [
            ...
            ProposalTask(number=3, denomination="Analysis & Results", description="Analysis and interpretation.", person_months=6, start_month=13, duration_months=12),
            ProposalTask(number=4, denomination="Dissemination & Management", description="Publications, coordination.", person_months=4, start_month=1, duration_months=dur),
        ]
