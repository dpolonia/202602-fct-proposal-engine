...
class DraftIdea(BaseModel):
    ...
    research_questions: list[str] = Field(
        default_factory=list, description="Draft research questions"
    )
    ...
    sdg_alignment: list[int] = Field(
        default_factory=list, description="UN SDG numbers (1-17), max 3"
    )
    ...
    def direct_costs(self) -> float:
        return sum(
            [
                ...
                self.missions_travel,
                self.equipment,
                self.consumables,
                self.services,
                self.registrations_publications,
                self.other,
            ]
        )
